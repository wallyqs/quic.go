...
func (c *Conn) handleShortHeaderPacket(
	...
	if err != nil {
		...
		wasQueued, err = c.handleUnpackError(err, p, "1-RTT")
		return false, err
	}
...
func (c *Conn) handleLongHeaderPacket(p receivedPacket, hdr *wire.Header) (wasProcessed bool, _ error) {
	...
	if err != nil {
		wasQueued, err = c.handleUnpackError(err, p, hdr.Type.String())
		return false, err
	}
	...
	if err := c.handleUnpackedLongHeaderPacket(packet, p.ecn, p.rcvTime); err != nil {
		return false, err
	}
	return true, nil
}

func (c *Conn) handleUnpackError(err error, p receivedPacket, pt string) (wasQueued bool, _ error) {
	switch err {
	...
	case handshake.ErrKeysNotYetAvailable:
		...
		// Try again later.
		c.tryQueueingUndecryptablePacket(p)
		return true, nil
...
func (c *Conn) handleUnpackedLongHeaderPacket(
	...
	isAckEliciting, _, _, err := c.handleFrames(packet.data, packet.hdr.DestConnectionID, packet.encryptionLevel, rcvTime)
	if err != nil {
		return err
	}
...
func (c *Conn) handleUnpackedShortHeaderPacket(
	...
	isAckEliciting, isNonProbing, pathChallenge, err := c.handleFrames(data, destConnID, protocol.Encryption1RTT, rcvTime)
	if err != nil {
...
func (c *Conn) tryQueueingUndecryptablePacket(p receivedPacket) {
	if c.handshakeComplete {
